...
| `deploy` (or `install`) exits with `Profile '<name>' requires the following vars in .env that are missing or empty:` followed by a per-var block | One or more vars that the active deploy profile needs are empty or malformed in `.env`. The validator lists every missing var at once with its type, description, and a copy-paste example. | Edit `.env` and set the listed vars, then re-run `./smartolt.sh deploy`. The validator catches: empty values, the `change-me-now` password sentinel, FQDNs without a dot or with whitespace, emails without `@` or `.`, images without `:tag`, passwords shorter than 8 characters. |
